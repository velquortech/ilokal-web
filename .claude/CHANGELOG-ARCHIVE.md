...
> **Entries older than 2026-08-07, moved here byte-for-byte from
> [`CHANGELOG.md`](CHANGELOG.md).** Nothing was summarised, merged, reordered
> or corrected — each entry reads exactly as it did when it was written,
> including the places where a later entry says an earlier one was wrong. That
> record is deliberate: keep it.
>
> **This file is NOT auto-loaded.** `CLAUDE.md` inlines only the live
> `CHANGELOG.md`; this one is read on request. The live file carries an index
> of **all** entries across both files, so start there to find what you need.
>
> **Newest first**, same as the live file. When entries rotate out of
> `CHANGELOG.md` they are pasted at the **top** of this file.
