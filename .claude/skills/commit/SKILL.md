...
2. Run lint **first**, then the full smoke check — detect from the repo root:
   - **Next.js / Node**: run `npm run lint -- --fix` first; if it exits non-zero, **abort and report lint errors before doing anything else**. Then run `npm run build` to catch type and build errors. Both must pass.
