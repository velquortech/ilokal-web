...
- **Proxy:** Single `proxy.ts` at repo root (Next.js 16 replaces `middleware.ts`) — refreshes session cookies for page routes and verifies JWTs for `/api/protected/**` via `supabase.auth.getUser()`. Sets `x-verified-user-id` header after verification (spoofing-safe); handlers reuse it to skip a redundant `getUser()` round-trip. Also gates `/api/admin/**` — verifies an authenticated `role === 'admin'` and returns JSON `401`/`403` (handlers still run their own `assertAuthorized({ roles: ['admin'] })` for defense in depth).
  ...
  - Web/admin handlers: call `assertAuthorized(req)` from `lib/utils/auth/`. Admin-only mutations under `/api/web/**` (e.g. business-types, business-categories) must pass `{ roles: ['admin'] }` — RLS alone is not the guard.
