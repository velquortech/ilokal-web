...
import { sendMenuFollowUpAction } from '../../actions/menuFollowUpActions';

/** Friendly copy for each skip reason the action can return. */
const SKIP_COPY: Record<string, string> = {
  ALREADY_HAS_MENU: 'This shop has added a menu — nothing to send.',
  NOT_ELIGIBLE: 'This shop is no longer eligible.',
  NO_EMAIL: 'This owner has no email on file.',
  RECENTLY_SENT: 'This owner was reminded recently.',
  NOT_FOUND: 'This shop could not be found.',
};

export function SendReminderButton({ businessId }: { businessId: string }) {
  ...
  const send = async () => {
    ...
    setSending(true);
    try {
      const res = await sendMenuFollowUpAction(businessId);
      if (!res.ok) {
