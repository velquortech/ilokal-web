-- ============================================================
-- A photo of its own for a deal
-- ------------------------------------------------------------
-- `coupons` had no image column at all, so a deal card was drawn from the
-- SHOP: `mobile_deals` projects `biz.logo_url` and `biz.interior_images[1]`.
-- That is a sensible fallback and it stays — this only lets an owner say
-- "this deal looks like THIS" when they have a picture of the actual thing.
--
-- Two changes, and the second is the one that matters: adding the column
-- alone would store a photo that nothing ever reads, which is a column that
-- lies. `mobile_deals` has to project it in the same migration.
--
-- ⚠️ `mobile_deals` is the RPC behind the app's Deals feed, and it exists
-- BECAUSE the previous in-Node version silently truncated at the PostgREST
-- 1000-row cap. Re-run `supabase/tests/mobile_deals_and_outbox.test.sql`
-- after touching it. The body below is the live definition with two lines
-- added; nothing else about the query changed.
--
-- Additive and nullable: every existing row reads NULL, every existing caller
-- ignores an unknown key, and no backfill is possible or wanted.
--
-- Rollback:
--   ALTER TABLE public.coupons DROP COLUMN image_url;
--   -- then restore the previous mobile_deals body from git history.
-- ============================================================
...
COMMENT ON COLUMN public.coupons.image_url IS
  'Bucket-relative path in `product-images` for this deal''s own photo. NULL '
  'means the card falls back to the shop logo / first interior image, which '
  'is what every deal did before this column existed. Never an absolute URL '
  '- see the 2026-08-06 gallery incident.';

-- CREATE OR REPLACE, never DROP + CREATE. A drop would take the EXECUTE
-- grants and the owner with it — and this function is SECURITY DEFINER and
-- anon-callable, so it would be silently unreachable for every anonymous
-- visitor until someone re-granted it. Replacing keeps both.
CREATE OR REPLACE FUNCTION public.mobile_deals(p_category text DEFAULT 'All'::text, p_search text DEFAULT ''::text, p_page integer DEFAULT 1, p_per_page integer DEFAULT 20)
 RETURNS jsonb
 LANGUAGE sql
