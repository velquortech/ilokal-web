-- Expose banner_url through nearby_businesses so the mobile app can show the
-- business's banner (cover) image on cards instead of the first interior shot.
--
-- WHY: the mobile Home "Nearest Businesses" cards and the Explore grid were
-- falling back to interior_images[0] as their hero photo. Businesses have a
-- curated banner_url (the web explore card already uses it as its card image),
-- but the RPC never emitted the column, so the mobile client couldn't see it.
-- This adds banner_url to the payload; the nearby route resolves the raw
-- storage path against the shop-banners bucket (same as logo_url/interior).
--
-- DROP first: adding a RETURNS TABLE column changes the OUT-param row type,
-- which CREATE OR REPLACE can't do (same pattern as 20260614000000).
