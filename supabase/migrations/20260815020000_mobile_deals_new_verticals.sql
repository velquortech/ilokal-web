-- ============================================================
-- mobile_deals: accept the 4 launch verticals' category keys
-- ------------------------------------------------------------
-- The Deals tab's category chips are derived from the app's MAIN_CATEGORIES,
-- which gained Entertainment / Health / Education / Home when the 4 launch
-- verticals (20260815000000) landed. mobile_deals maps the mobile category
-- KEY to a business_types.name; without the new branches a chip's key maps to
-- NULL and the tab silently returns no deals for that vertical.
--
-- Pure CREATE OR REPLACE of the function from 20260807120000 (the coupon
-- image version) with the CASE extended by four branches. The rest of the
-- body is untouched, so the featured/flash/explore logic is byte-identical.
-- ============================================================
...
WITH params AS (
  SELECT
    ...
    CASE p_category
      ...
      WHEN 'Entertainment' THEN 'Entertainment & Events'
      WHEN 'Health'        THEN 'Health & Wellness'
      WHEN 'Education'     THEN 'Education & Learning'
      WHEN 'Home'          THEN 'Home & Property Services'
      ELSE NULL
    END                                               AS v_type
),
